...
if (userArgs.length === 2 && userArgs[1] == '-r') {
	if ( ! input.match(/[a-zA-Z](?:\.)[a-zA-Z]/)) {
		...
		setTimeout(function() {
			open('https://api.domainr.com/v1/api/register?domain='+input+'&client_id=is-available');
		}, 1000);
		return;
	}
}

path = 'https://api.domainr.com/v1/api/json/search?q='+input+'&client_id=is-available';
